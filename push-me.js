...
async function main() {
  ...
  if (!service.config.couple?.my_pushkey) {
    console.error('❌ 未配置 my_pushkey');
    process.exit(1);
  }
  ...
  const { title, content } = service.buildMyMessage(weatherResult, city);
  console.log(`📤 推送给自己（宝宝）...`);

  const ok = await service.sendToPushkey(service.config.couple.my_pushkey, title, content);
  if (ok) {
    console.log(`✅ 推送成功！`);
    process.exit(0);
  } else {
